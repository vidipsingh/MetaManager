...
const DashboardContent = ({ onTodoClick, teamMembersCount }: Props) => {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.id) {
      Promise.all([fetchTodos(), fetchEvents()]).finally(() => setIsLoading(false));
    }
  }, [session, status]);
            ...
            <div className="space-y-4">
              {todos
                ...
                .map((todo) => (
                  <div
                    key={todo.id}
                    className="p-4 rounded-lg border dark:border-gray-700 cursor-pointer hover:bg-zinc-100 dark:hover:bg-gray-900"
                    onClick={onTodoClick}
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span
                        className={`px-2 py-1 rounded text-xs ${getPriorityColor(todo.priority)}`}
                      >
                        {todo.priority}
                      </span>
                    </div>
