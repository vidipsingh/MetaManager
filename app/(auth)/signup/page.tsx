    ...
    try {
      ...
      if (data.message) {
        ...
        if (signInResult?.ok) {
          ...
          setMessage("Sign-in failed after registration");
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      if (result?.ok) {
        ...
        setMessage(result?.error || "An error occurred during Google sign-up");
        setAlertSeverity("error");
        setShowAlert(true);
      }
    } catch {
            ...
            <div className="w-full p-5 border border-gray-400 rounded-md bg-white/60 dark:bg-black/90">
              <div>
                ...
                <h1 className="my-3 dark:text-white text-black">
                  ...
                  <Link href="/login">
                    <span className="dark:text-purple-300 text-purple-800 hover:text-purple-950 underline dark:hover:text-purple-400 cursor-pointer ml-0.5">
                      Sign In
                    </span>
                  </Link>
                </h1>
